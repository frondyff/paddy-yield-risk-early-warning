...
## Executive Summary
This is not a universal yield optimizer. It is a governed decision-support system that helps teams decide:
...
Model evaluation update applied:
- Primary performance reporting now uses a trial-aware train/validation/test split (60/20/20), with final reporting on the locked test set.
- Final selected model: ExtraTrees (test R2 = 0.4792, RMSE = 1002.02, MAE = 749.09).
...
Rice outcomes across geographies are driven by interactions between:
...
The framework's business-oriented workflow:
...
|---|---:|---|---|
| R1 - Benin | -1,198.8 | [-1,935.5, -378.7] | Avoidance pilot (prevent likely loss) |
| R1 - Nigeria | -805.9 | [-903.8, -732.5] | Avoidance pilot |
| R2 - Nigeria | +545.1 | [205.3, 998.2] | Controlled positive pilot |
| R5 - Benin | +1,473.8 | [1,182.8, 1,988.8] | Controlled positive pilot |
...
`Gross impact (USD) = Eligible hectares x Effect (kg/ha) x Farm-gate price (USD/kg) x Adoption rate`

`Avoided loss (USD) = Eligible hectares x |Negative effect| x Farm-gate price x Compliance rate`

Illustrative magnitude at 1,000 ha and USD 0.25-0.35/kg:
- R5-Benin: USD 0.37M-0.52M gross upside
...
- Pilot wave 1: R5-Benin, R2-Nigeria
